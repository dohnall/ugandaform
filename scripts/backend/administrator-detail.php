...
if($user_id) {
	$query = "SELECT * FROM ".DBPREF."user WHERE admin = 'Y' AND user_id = ".$user_id;
	$data = dibi::query($query)->fetch();
	if(!$data) {
		Common::redirect(ROOT.'admin/administrator-detail');
	}
	$new = false;
} else {
	$new = true;
...
if(isset($_POST['save'])) {
	...
	Common::redirect(ROOT.'admin/administrator');
}
